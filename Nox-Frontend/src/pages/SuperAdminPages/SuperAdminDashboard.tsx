...
} from "@/components/modals/SuperAdminModals/DepartmentModal";
...
export default function Index() {
  ...
  const stats = [
    ...
    {
      icon: (
        <img src="departmentsIcon.png" alt="" className="w-[70px] h-[60px]" />
      ),
      label: "Departments",
  ...
  const getDepartmentUsers = (departmentId: number) => {
    return users.filter(
      (user) => user.departmentId === departmentId && user.isActive
    );
  };
    ...
    <SuperAdminHeader>
      <main className="px-6 lg:px-8 space-y-8 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {departments.map((dept) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-12 h-12 rounded-full ${color} flex items-center justify-center shrink-0 shadow-sm`}
                    >
        ...
        <DepartmentModal
          ...
          users={
            modalType === "view-users" && selectedDept
              ? getDepartmentUsers(selectedDept.id)
              : []
          }
